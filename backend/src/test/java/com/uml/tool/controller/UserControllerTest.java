...
import com.uml.tool.DTO.ChangePasswordDTO;
...
class UserControllerTest {
    ...
    void testUpdateUser_DuplicateUsername() throws Exception {
        ...
        when(userService.getByUsername(any())).thenReturn(java.util.Optional.of(existing));
        mockMvc.perform(put("/api/users/test@example.com")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{" +
                        "\"email\":\"test@example.com\"," +
    ...
    void testUpdateUser_DuplicateEmail() throws Exception {
        com.uml.tool.model.UserLoginDetails existing = new com.uml.tool.model.UserLoginDetails();
        existing.setEmail("other@example.com");
        when(userService.getByUsername(any())).thenReturn(java.util.Optional.empty());
        when(userService.getByEmail(any())).thenReturn(java.util.Optional.of(existing));
        mockMvc.perform(put("/api/users/test@example.com")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{" +
