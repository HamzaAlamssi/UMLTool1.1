...
const styles = {
    ...
    toolbar: {
        ...
        boxShadow: '2px 0 8px rgba(0,0,0,0.1)', // Add shadow on right edge
        zIndex: 10,
    },
    button: {
        padding: '7px 18px',
...
const UmlEditor = forwardRef(({ initialModel }, ref) => {
    ...
    function getNameAndIconPosition(ctx, obj) {
        ...
        if (obj.type === 'class') {
            ...
            'node', 'artifact', 'device', 'component', 'interface', 'port'
        ].includes(obj.type)) {
    ...
    function handleCanvasClick(e) {
        ...
        for (let i = classes.length - 1; i >= 0; i--) {
            ...
            if (mx >= editIcon.x && mx <= editIcon.x + editIcon.w && my >= editIcon.y && my <= editIcon.y + editIcon.h) {
                showModal('Edit Name', obj.name, val => {
                    if (val) setClasses(prev => prev.map(c => c.id === obj.id ? { ...c, name: val } : c));
                });
                return;
            }
        ...
        if (relationMode) {
            ...
            } else if (pendingRelation.fromId !== obj.id) {
                if (!relationships.some(r => r.fromId === pendingRelation.fromId && r.toId === obj.id && r.type === pendingRelation.type)) {
                    setRelationships(prev => [...prev, { fromId: pendingRelation.fromId, toId: obj.id, type: pendingRelation.type }]);
                }
                ...
                for (let j = 0; j < obj.attributes.length; j++) {
                    ...
                    if (mx >= obj.x + obj.width - 52 && mx <= obj.x + obj.width - 28 && my >= attrY + 2 && my <= attrY + 26) {
                        showModal('Edit Attribute', obj.attributes[j].name, ({ name, visibility }) => {
                            if (name) setClasses(prev => prev.map(c =>
    ...
    }, [classes, relationships, hoveredShape]);
    ...
    const handleAddClass = () => {
        showModal('Class Name', '', name => {
            ...
            setClasses(prev => [...prev, UmlClass(x, y, name)]);
        });
    };
    ...
    const handleAddActor = () => {
        showModal('Actor Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, Actor(x, y, name)]);
        });
    };
    const handleAddUseCase = () => {
        showModal('Use Case Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, UseCase(x, y, name)]);
        });
    };
    const handleAddSystem = () => {
        showModal('System Name', '', name => {
            ...
            setClasses(prev => [...prev, SystemBoundary(x, y, name)]);
        });
    };

    // === Toolbar actions for more element types ===
    const handleAddAction = () => {
        showModal('Action Name', '', name => {
            if (!name) return;
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, ActionNode(x, y, name)]);
        });
    };
    const handleAddInitial = () => {
        const x = 100 + Math.random() * 600;
        const y = 100 + Math.random() * 400;
        setClasses(prev => [...prev, InitialNode(x, y)]);
    };
    const handleAddFinal = () => {
        const x = 100 + Math.random() * 600;
        const y = 100 + Math.random() * 400;
        setClasses(prev => [...prev, FinalNode(x, y)]);
    };
    const handleAddFork = () => {
        showModal('Fork/Join Orientation (h/v)', 'h', dir => {
            ...
            setClasses(prev => [...prev, ForkNode(x, y, orientation)]);
        });
    };
    const handleAddDecision = () => {
        const x = 100 + Math.random() * 600;
        const y = 100 + Math.random() * 400;
        setClasses(prev => [...prev, DecisionNode(x, y)]);
    };
    const handleAddNode = () => {
        showModal('Node Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, NodeElement(x, y, name)]);
        });
    };
    const handleAddArtifact = () => {
        showModal('Artifact Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, ArtifactElement(x, y, name)]);
        });
    };
    const handleAddDevice = () => {
        showModal('Device Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, DeviceElement(x, y, name)]);
        });
    };
    const handleAddComponent = () => {
        showModal('Component Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, ComponentElement(x, y, name)]);
        });
    };
    const handleAddInterface = () => {
        showModal('Interface Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, InterfaceElement(x, y, name)]);
        });
    };
    const handleAddPort = () => {
        showModal('Port Name', '', name => {
            ...
            const x = 100 + Math.random() * 600;
            const y = 100 + Math.random() * 400;
            setClasses(prev => [...prev, PortElement(x, y, name)]);
        });
    };
    ...
    //   hoverTimeout.current = setTimeout(() => setToolbarVisible(false), 250);
    // }
        ...
        <>
            ...
            <div id="canvas-container"/*style={styles.container}*/>
                <canvas
                    ...
                    // ...add mouse event handlers here, e.g. onMouseDown, onMouseMove, etc...
                />
